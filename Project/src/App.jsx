...
// Navbar component, accepting onStartScalpAnalysis as a prop
const Navbar = ({ onStartScalpAnalysis }) => {
  return (
    <nav className="hidden md:flex fixed top-0 left-0 w-full bg-white shadow-xl z-50 px-6 py-3 items-center justify-between rounded-b-xl">
      <div className="flex items-center space-x-2 transform hover:scale-105 transition-transform duration-300 cursor-pointer">
        <img
          src="./LOGO.JPG" // Placeholder for logo image, consistent with theme
...
// SectionCard component for general content sections
...
const HeroSection = ({ onStartScalpTest, onBookConsultation }) => (
  <section id="home" className="text-white py-20 flex items-center justify-center min-h-screen" // Added flex, items-center, justify-center, min-h-screen
    style={{
      backgroundImage: 'url(https://plus.unsplash.com/premium_photo-1677610855713-82e8649e6d73?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8aGFpcmxvc3MlMjBhbmQlMjBkb2N0b3J8ZW58MHx8MHx8fDA%3D)', // Replaced with a relevant Unsplash image
      backgroundSize: 'cover',
      backgroundPosition: 'center',
    }}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-12 items-center z-10"> {/* Added z-10 to keep content above background */}
      <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button onClick={onBookConsultation} className="border-2 border-white text-black px-8 py-4 rounded-full font-semibold hover:bg-white hover:text-green-600 transition duration-300">
            Book Consultation
          </button>
        </div>
      </div>
...
// Scalp Analyzer component
const ScalpAnalyzer = ({ onFinishQuiz }) => {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(-1); // -1 for initial state, 0 for first question
  ...
  const selectAnswer = () => {
    if (currentQuestionIndex < totalQuestions - 1) {
      ...
      onFinishQuiz(); // Call parent function when quiz is complete
    }
  };
    ...
    <section id="scalp-analysis" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-gradient-to-r from-teal-500 to-lime-500 rounded-2xl p-8 text-">
            <div className="mb-6">
              ...
              <div className="w-full bg-white bg-opacity-30 rounded-full h-2">
                <div className="progress-bar bg-green-400 h-2 rounded-full" style={{ width: `${progressPercent}%` }}></div>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {currentQuestion.options.map((option, index) => (
                      <button
                        ...
                        className="bg-white bg-opacity-20 hover:bg-opacity-30 text-black px-6 py-3 rounded-full font-semibold transition duration-300"
                      >
                        {option}
                      </button>
                    ))}
...
// Why Choose FolliclePlus Section
...
// Blood Test & Consultation Section
      ...
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div>
          <svg width="500" height="400" viewBox="0 0 500 400" className="w-full h-auto">
            ...
            <circle cx="150" cy="150" r="30" fill="#a7f3d0"/> {/* Light green */}
            <rect x="120" y="180" width="60" height="80" rx="10" fill="#2dd4bf"/> {/* Teal */}
            <rect x="130" y="190" width="40" height="5" fill="white"/>
            <circle cx="350" cy="150" r="30" fill="#bef264"/> {/* Lime green */}
            <rect x="320" y="180" width="60" height="80" rx="10" fill="#84cc16"/> {/* Green */}
            ...
            <rect x="210" y="155" width="50" height="6" fill="#10b981"/> {/* Green */}
            <rect x="210" y="170" width="65" height="6" fill="#ef4444"/>
            <rect x="210" y="185" width="55" height="6" fill="#10b981"/> {/* Green */}
            ...
            <circle cx="400" cy="300" r="15" fill="#2dd4bf"/> {/* Teal */}
        ...
        <div>
          ...
          <button onClick={onBookTest} className="bg-green-600 text-black px-8 py-4 rounded-full font-semibold hover:bg-green-800 transition duration-300">
...
// Main App Component
const App = () => {
  // State for controlling the modal visibility and content
  const [showModal, setShowModal] = useState(false);
  const [modalContent, setModalContent] = useState({ title: '', message: '' });

  // Ref to scroll to the Scalp Analysis section
  ...
  // Handler for starting scalp analysis, scrolls to the section
  const handleStartScalpAnalysis = () => {
    if (scalpAnalysisRef.current) {
      scalpAnalysisRef.current.scrollIntoView({ behavior: 'smooth' });
      // Optionally start the quiz directly after scrolling
      // setTimeout(() => document.querySelector('#scalp-analysis button').click(), 500);
    }
  };

  // Handlers for various actions, using the custom modal
  ...
  // Effect for smooth scrolling when navigation links are clicked
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-inter">
      {/* Inline style block for global CSS, including font import and theme-specific gradients */}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
        body { font-family: 'Inter', sans-serif; }
        /* Removed .hero-gradient from here as it's now applied inline with backgroundImage */
